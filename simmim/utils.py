...
def load_checkpoint(config, model, optimizer, lr_scheduler):
    print(">>>>>>>>>> Resuming from {config.MODEL.RESUME} ..........")
    ...
    if not config.EVAL_MODE and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
        ...
        print("=> loaded successfully '{config.MODEL.RESUME}' (epoch {checkpoint['epoch']})")
...
def auto_resume_helper(output_dir):
    ...
    print("All checkpoints founded in {output_dir}: {checkpoints}")
    if len(checkpoints) > 0:
        latest_checkpoint = max([os.path.join(output_dir, d) for d in checkpoints], key=os.path.getmtime)
        print("The latest checkpoint founded: {latest_checkpoint}")
